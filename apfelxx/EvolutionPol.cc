...
int main()
{
  ...
  const std::vector<std::string> fnamev{
    ...
    "../results/apfelxx-table17-part1.csv",
    "../results/apfelxx-table17-part2.csv",
  ...
  for (int i = 0; i < (int) ptov.size(); i++)
    {
      ...
      const apfel::TabulateObject<apfel::Set<apfel::Distribution>> TabulatedPDFs{*BuildDglap(InitializeDglapObjectsQCDpol(g, Massesv[i]), apfel::LHToyPDFsPol, mu0, ptov[i], [&] (double const& mu) -> double{ return Alphas.Evaluate(mu); }, xiv[i]), 50, 1, 1000, 3};
