...
*    polarized splitting functions  P2NS  in N-space for  
*    NF = NFMIN ... NFMAX= 3...6  massless quark flavours in the MS(bar) 
*    scheme. The coupling constant is normalized as a_s = alpha_s/(4*pi). 
*
...
* ---------------------------------------------------------------------
*
* IB  The parametrized n_f^{0,1} components of DP_ns^(2)i, i = +, -, v
*     as given in S. Moch, J. Vermaseren and A. Vogt, hep-ph/0403192
*     Note that DP_ns^(2) +- = P_ns^(2) -+
*   
*
       PM20 = + 1174.898 * A0 + 1295.384 + 714.1 * B1 - 522.1 * C3
     1        + 243.6 * C2 - 3135.* C1 + 1641.1 * C0 + 1258.* D1
     2        + 294.9 * D2 + 800/27.D0 * D3 + 128/81.D0 * D4
     3        + 563.9 * E1 + 256.8 * E2
       PM21 = - 183.187 * A0 - 173.927 - 5120/81.D0 * B1
     1        + 44.79 * C3 + 72.94 * C2 + 381.1 * C1 - 197.0 * C0
     2        - 152.6 * D1 - 2608./81.D0 * D2 - 192./81.D0 * D3
     3        - 56.66 * E1 - 1.497 * D31 
*
       PP20 = + 1174.898 * A0 + 1295.470 + 714.1 * B1 - 433.2 * C3
     1        + 297.0 * C2 - 3505.*C1 + 1860.2 * C0 + 1465.2 * D1
     2        + 399.2 * D2 + 320./9.D0 * D3 + 116./81.D0 * D4
     3        + 684.0 * E1 + 251.2 * E2
       PP21 = - 183.187 * A0 - 173.933 - 5120/81.D0 * B1
     1        + 34.76 * C3 + 77.89 * C2 + 406.5 * C1 - 216.62 * C0
     2        - 172.69 * D1 - 3216./81D0 * D2 - 256./81.D0 * D3
     3        - 65.43 * E1 - 1.136 * D31
*
* IB  Mellin moments of the parametrized P_ns^(2)S, 
*     as given in S. Moch, J. Vermaseren and A. Vogt, hep-ph/1506.04517

       PS2  = - 42.97 * (D2-D21)-29.29 * (D1-D11) + 179.1 * (C0-C1)
     1        + 117.8 * (C1-C2) -385.5 * (C2-C3) + 75.94 * (C3-C4)
     2        + 8.818 * (D21-D22) + 460.8* (D11-D12) + 2.681 * (B1-B11)
     2        + 0.0001 ! delta(1-x) term
